...
function initLoginApp () {
    ...
    if (!isLoginPage(pagina)) {
        ...
        // Após login, redireciona para a página de início (ou para onde o usuário tentou acessar)
        RETURN_URL = sessionStorage.getItem('returnURL') || HOME_URL;
...
function addUser (nome, login, senha, email) {
    ...
    fetch(apiUrl('usuarios'), {
        ...
        .then(response => response.json())
        .then(data => {
                // Adiciona o novo usuário retornado pelo servidor (contém id)
                db_usuarios.push (data);
                displayMessage("Usuário inserido com sucesso", "sucesso");
            })
        .catch(error => {
            console.error('Erro ao inserir usuário via API JSONServer:', error);
            displayMessage("Erro ao inserir usuário", "erro");
        });
}
